...
- ✅ Честно сообщает об ошибках: в режимах по умолчанию и `-Mp4` «успех» без созданного файла невозможен (в `-Video`/`-Audio` ненайденный файл даёт предупреждение при коде `0`)
