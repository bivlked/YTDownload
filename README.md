...
Перед каждым скачиванием скрипт валидирует URL, проверяет здоровье cookies и только затем запускает yt-dlp с проверенными флагами. После скачивания честно проверяет результат: код возврата 0 без созданного файла не считается успехом.
...
3. **Онлайн-проверка cookies** (условная): выполняется только если локальная проверка нашла проблемы. Быстрая, без скачивания медиа (`--skip-download`). Если ошибка похожа на проблему cookies - показывает инструкцию по их обновлению; иначе честно перечисляет другие возможные причины (видео удалено/приватное, устаревший yt-dlp).
4. **Валидация URL**: проверка домена и пути; при ошибке - примеры правильных форматов.
...
Момент скачивания (передаётся `--no-mtime`), а не дата публикации ролика. Так файлы корректно сортируются «по новизне» в проводнике.
...
| `--no-mtime` | Время файла = момент скачивания, а не дата публикации ролика |
...
Нашли баг или хотите предложить улучшение? Создайте [Issue](https://github.com/bivlked/YTDownload/issues) или Pull Request! При сообщении о баге приложите: версию PowerShell (`$PSVersionTable.PSVersion`), версию yt-dlp (`.\yt-dlp.exe --version`) и полный текст ошибки.
